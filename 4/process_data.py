...
def impute_missing_values_with_statistics(df: pd.DataFrame, features: list, strategy="mode") -> pd.DataFrame:
    if strategy == "mode":
...
def impute_missing_values(
    ...
    numerical_constant_imputers: list,
) -> pd.DataFrame:
    ...
    df = impute_missing_values_with_statistics(df, features=categorical_features)
    for imputer in numerical_constant_imputers:
        df = impute_missing_values_with_constant(
            df,
...
if __name__ == "__main__":
    ...
    numerical_constant_imputers = [{"impute_value": 0}]
